...
방송하는 **내 PC 안에서만** 도는 로컬 서버입니다(`127.0.0.1`). 채팅을 읽으려고 치지직과 통신하고, 시작할 때 새 버전 확인을 위해 GitHub에 한 번 연결하는 것 외에는 오버레이 설정 같은 데이터를 외부로 보내지 않습니다. (버전 확인은 `config.json`의 `updateCheckEnabled`를 `false`로 두면 꺼집니다.)
...
3. **파일 업로드** 로 이미지/영상을 올립니다. (또는 `assets` 폴더에 파일을 직접 넣고 파일명을 입력)
   ...
   - 작은 이미지나 짧은 영상 하나로 먼저 테스트해보면 좋습니다.
...
- 새 버전 알림 → 새 버전이 나오면 컨트롤 페이지 상단에 배너로 알려줍니다. **릴리스 페이지 열기**로 받은 새 폴더로 바꿀 때 `config.json`·`credentials.json`·`chzzk_tokens.json` 은 덮어쓰지 마세요(설정·로그인이 유지됩니다). 알림이 필요 없으면 `config.json` 의 `updateCheckEnabled` 를 `false` 로 두면 됩니다.
